...
// New table to track each user request (for rate limiting / usage tracking)
export const userRequests = createTable(
  ...
  (t) => ({
    userIdIdx: index("user_request_user_id_idx").on(t.userId),
    createdAtIdx: index("user_request_created_at_idx").on(t.createdAt),
  }),
);
...
export const messages = createTable(
  "message",
  {
    ...
    // Optional structured annotation objects emitted during reasoning loop (e.g. NEW_ACTION steps)
  ...
  (t) => ({
    ...
    chatOrderIdx: index("message_chat_id_order_idx").on(t.chatId, t.order),
  }),
);
