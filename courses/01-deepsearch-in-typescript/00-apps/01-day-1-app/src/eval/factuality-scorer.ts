...
/**
 * Factuality checking using an LLM-as-a-judge approach.
 * Prompt adapted from autoevals factuality template.
 */
...
(E) The answers differ, but these differences don't matter from the perspective of factuality.`,
  ...
  const score = scores[object.answer] ?? 0;
  return {
    score,
    metadata: {
...
export const Factuality = createScorer<string, string, string>({
  ...
  scorer: async ({ input, expected, output }) => {
    // input is the user question string provided in eval data
