...
/**
 * Answer Relevancy scorer (inspired by Mastra's implementation).
 *
 * Approach:
 * 1. Break model output into meaningful statements (first LLM call).
 * 2. For each statement, judge relevance to the original input question (second LLM call).
 * 3. Map verdicts (yes=1, unsure=0.5, no=0) and average for final score.
 *
 * Returned metadata includes the statements, their verdicts, reasons, and the per-statement numeric scores.
 */
...
// First phase: extract statements.
async function extractStatements(output: string) {
  ...
  const { object } = await generateObject({
    ...
    schema: z.object({
      statements: z.array(z.string()).describe("List of statements."),
    }),
  });
  // Defensive: filter empties & trim
  return (object.statements || []).map((s) => s.trim()).filter(Boolean);
}

// Second phase: evaluate relevance per statement.
async function judgeRelevance(input: string, statements: string[]) {
  const { object } = await generateObject({
    model: factualityModel,
    ...
    schema: z.object({
      verdicts: z
        ...
        .describe("Verdict list matching statements length"),
    }),
  });
...
export const AnswerRelevancy = createScorer<string, string, string>({
  ...
  scorer: async ({ input, output }) => {
    const answer = output ?? "";
    const statements = await extractStatements(answer);

    // If no statements (empty output), trivially score 0.
    ...
    const verdicts = await judgeRelevance(String(input), statements);

    // Ensure length alignment; if mismatch, truncate/pad with no.
    const normalizedVerdicts = statements.map(
      (_, idx) =>
        verdicts[idx] ?? {
          verdict: "no",
          reason: "Missing verdict from model; defaulted to no.",
        },
    );

    const mapping: Record<string, number> = { yes: 1, unsure: 0.5, no: 0 };
    const perStatementScores: number[] = normalizedVerdicts.map(
      (v) => mapping[v.verdict] ?? 0,
    );
    const total = perStatementScores.reduce((a: number, b: number) => a + b, 0);
    const avg =
      perStatementScores.length > 0 ? total / perStatementScores.length : 0;
